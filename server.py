...
def index():
    ...
    return render_template('index.html' ,server_ip=get_server_ip(), server_info=get_server_info(), logged_in='username' in session, username=session.get('username'), user_avatar="https://via.placeholder.com/40", appstore=APPSTORE,websites=WEBSITES, notification=notification ,items=items, current_year=datetime.now().year) 


#### notic
def get_notification():
    try:
        response = requests.get("https://raw.githubusercontent.com/amatak-org/opensource-app/refs/heads/main/kpanel_notic")
...
def notic():
    ...
    return render_template('notic.html')
